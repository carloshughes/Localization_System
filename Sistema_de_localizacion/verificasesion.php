...
$fila=mysql_fetch_array($rst);
	...
	if ($fila["nombre"]!="")
		...
		$fecha = date ("Y-m-d");
		$hora = date("h:i:s"); 
		$sql = "INSERT INTO entrada (usuario, nombre, hora, fecha) "; 
		$sql.= "VALUES ('". $_REQUEST["usuario"] ."','".$fila["nombre"]."','$hora','$fecha')"; 
		//$es = mysql_query($sql, $conexion) or die(" header ("Location:Error_conexion.php"); Usuario ya Conectado"); 		
		$es = mysql_query($sql, $conexion) or die (header ("Location:Error_conexion.php")); 
	}
...
<p>&nbsp;</p>
<p align="center"><span class="style4"><img src="imagenes/logo.JPG" width="398" height="295" /></span></p>
<p>&nbsp;</p>
<p>&nbsp;</p>
