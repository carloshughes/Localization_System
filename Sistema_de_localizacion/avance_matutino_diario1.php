...
<table width="1031" align="center">
  ...
  <tr>
	<?php 	
		...
		$sql= "SELECT  count(*) contactos FROM base_captura WHERE eslabon='".$fila["usuario"]."' and fecha ='".$fecha = date ("Y-m-d")."' and status_cuenta='CONTACTO'";
		$rs = mysql_query($sql, $conexion);
		$count = mysql_fetch_array($rs);
		
		//consulta para saber cuentas cuentas, han sido trabajadas por cada asesor
		$sql_1= "SELECT  count(*) avance FROM base_captura WHERE eslabon='".$fila["usuario"]."' and fecha ='".$fecha = date ("Y-m-d")."'";
